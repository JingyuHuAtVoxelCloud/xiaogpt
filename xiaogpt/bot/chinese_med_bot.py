...
class ChineseMedBot(ChatHistoryMixin, BaseBot):
    name: ClassVar[str] = "Chinese Medicine"
    ...
    def _make_query_engine(self, sess: httpx.AsyncClient, stream=False):
        ...
        embed_model = HuggingFaceEmbedding(
            model_name="/mnt/nas_ssd_data/jyhu/llm/models/bge-large-zh-v1.5"
            )
        Settings.embed_model = embed_model
        Settings.llm = llm
        # check if storage already exists
        PERSIST_DIR = "xiaogpt/rag/classic_medicine_data/storage"
        if not os.path.exists(PERSIST_DIR):
            # load the documents and create the index
            documents = SimpleDirectoryReader("xiaogpt/rag/classic_medicine_data/data").load_data()
...
async def main():
    ...
    bot = ChineseMedBot.from_config(config)
    # 询问问题
    response = await bot.ask("什么是石斛？")
